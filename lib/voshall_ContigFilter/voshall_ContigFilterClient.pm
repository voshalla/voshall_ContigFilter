package voshall_ContigFilter::voshall_ContigFilterClient;
...
voshall_ContigFilter::voshall_ContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => voshall_ContigFilter::voshall_ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a voshall_ContigFilter.FilterContigsParams
...
# Authentication: required
	...
	method => "voshall_ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "voshall_ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for voshall_ContigFilter::voshall_ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "voshall_ContigFilter::voshall_ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package voshall_ContigFilter::voshall_ContigFilterClient::RpcClient;
